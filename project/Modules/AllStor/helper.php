...
function allstor_get_menu_top($parent = NULL, int $menu_id = 0){
  ...
  if (empty($menu_data) === false) {
    ...
    foreach ($menu_data as $key => $value) {
	    $active = ($a1 == 0) ? (rtrim(url()->current(), '/') == rtrim($value->url, '/')) ? 'active' : '' : '' ;	    
	    $a1++;
	    $menu_html .= '<li class="allstor_child_class"><a href="'.$value->url.'" class="'.$active.'" >'.$value->title.'</a>';
...
function allstor_get_menu_catalog($parent = NULL, int $menu_id = 0){
  ...
  if (empty($menu_data) === false) {
  	$a1 = 0;
    foreach ($menu_data as $key => $value) {
      $activeurl = (rtrim(url()->current(), '/') == rtrim($value->url, '/')) ? 'active' : '';
	    $active = ($a1 == 0) ? 'class="'.$activeurl.'"' : '' ;	    
	    $a1++;
	    $menu_html .= '<li><a href="'.$value->url.'" '.$active.'>'.$value->title.'</a><i class="fa fa-angle-right"></i>';
